...
* After test notes from Chap: 

0. Need to focus on the interpretation of standard error, lost points for that 
   + margin of error is approximately half the confidence interval 
   + confidence interval is approximately 2 times the standard error 
   + So the standard error is half the margin of error 
   
1. focus on how weighted least squared affect standard error of the estimate 

2. study that in models with rates, variance is not constant since it is a function of some other things, 
  so we most likely will always have a non-constant variance. 
  Need to dig into it and include into the final guide to regression 
  
3. Focus on type of sum of squares, likely covered in book chapter 
  Important to know because the standard method used by the software
  will change what results we see and what we will report 

4. We have to be more precise with our language 

  + no definitive statements can be made about the graphs, 
    i.e. can't say variance is constant or not, can only say it is "likely constant" ot "not" 
    
  + when reporting statistical test results, we have to be more clear with the accepted level confidence we use, 
    beucase not everyone uses $\alpha=0.05$ all the time 
