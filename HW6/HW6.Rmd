...
We can see that we should fit the plynomial regression model because the smooth line shows a 
...
* An addition of a quadratic term turns the effect of age on birth weight from a straight line to the 
  ...
  + A very wide parabola usualyy does not indicate a strong effect, and visually it should apprear 
...
partial coefficient of determination to desribe their predictive power in more depth. 
...
* Extra $R^2$ of mother's age squared after the addition of gestational weeks and linear term for 
  mother's age is: `r round(Rs_3, 4)`, which is extremely low
...
* Therefore, same conclusions apply for the correaltion transformed regression model and orignal scale
  model. 
...
model, and denote predcitors which are statistically significant at the new level with a "*". 
...
Next we would like to implement **HOLM Adjustments**. The procedure involves iterrative evaluation 
...
* next predictor will be tested at $0.05 / 7 =$ `r round(0.05 / (n_predictors-1), 4)`. 
    + So, each time we declare a predictor significant, we increase the denominator for the new 
      cutoff by +1. 
...
Once again, Arm 6 indicator variable is the only statistically signifcant predictor. 
...
Therefore, after a series of p-value adjustemtns we can conclude that we do not have any false 
positive statsistically signifiacnt predictors, and we can state that Arm 6 is the only statistically significant predcitor after adjusting for other covaraites. 
...
As a result of the review, there are no statistically signifacnt predictors of Y2 when all variables 
are included. 
...
using built in fucntions, we ontain a subset of candidate models. We will evaluate these 
candidate models using adjusted R-squared and AIC metrics. There two metrics should provide us with 
the simplest and most effective models that explain the higest proportion of Y1 variance, after 
...
It apprears that model 3 is perhaps the best possible model that we can employ to explain variation 
